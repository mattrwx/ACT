...
namespace threads
{
    constexpr uint32_t ThreadQuerySetWin32StartAddress = 9;
    ...
    static NtQueryInformationThread_t get_nt_query_information_thread()
    {
        static NtQueryInformationThread_t nt_query = []() -> NtQueryInformationThread_t {
            HMODULE ntdll = GetModuleHandleA("ntdll.dll");// maybe it can be easly bypass but idk
            if (!ntdll)
                return nullptr;
    ...
    void validate_thread_start_addresses()
    {
        ...
        auto nt_query = get_nt_query_information_thread();
        if (!nt_query)
        {
