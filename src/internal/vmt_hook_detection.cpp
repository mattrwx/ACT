#include "vmt_hook_detection.hpp"
#include "../cache.hpp"
#include "../flags.hpp"
#include "../utils.hpp"
#include <print>
...
typedef HRESULT(WINAPI *d3d11create_t)(IDXGIAdapter *, D3D_DRIVER_TYPE, HMODULE, UINT, const D3D_FEATURE_LEVEL *, UINT, UINT, const DXGI_SWAP_CHAIN_DESC *, IDXGISwapChain **, ID3D11Device **, D3D_FEATURE_LEVEL *, ID3D11DeviceContext **);
...
bool d3d8_init(d3d8_info &info)
{
    ...
    if (!info.module) return false;

    auto create = reinterpret_cast<void *(__stdcall *)(UINT)>(
        GetProcAddress(info.module, "Direct3DCreate8"));
    if (!create) return false;

    info.d3d8 = create(220);
    if (!info.d3d8) return false;
...
bool d3d9_init(d3d9_info &info)
{
    ...
    if (FAILED(create(D3D_SDK_VERSION, &info.d3d9ex))) return false;
...
bool d3d10_init(d3d10_info &info)
{
    ...
    if (FAILED(create_factory(fac_iid, (void **)&factory))) return false;
...
bool d3d12_init(d3d12_info &info)
{
    ...
    if (!info.module || !info.d3d12_module) return false;
    ...
    if (FAILED(create_factory(__uuidof(IDXGIFactory2), (void **)&factory))) return false;
...
void cache_swapchain(void* swap_ptr, bool present1)
{
    ...
    if (SUCCEEDED(swap->GetDevice(__uuidof(ID3D12CommandQueue), (void **)&owner)))
    {
        ...
        if (present1) cache::game_d3d12_swap1 = swap_ptr;
        owner->Release();
        return;
    }
    ...
    if (SUCCEEDED(swap->GetDevice(__uuidof(ID3D11Device), (void **)&owner)))
    {
        ...
        if (present1) cache::game_d3d11_swap1 = swap_ptr;
        owner->Release();
        return;
    }
    ...
    if (SUCCEEDED(swap->GetDevice(__uuidof(ID3D10Device), (void **)&owner)))
    {
        ...
        if (present1) cache::game_d3d10_swap1 = swap_ptr;
        owner->Release();
        return;
    }
}
...
void get_d3d8_offsets(d3d8_offsets *offsets)
{
    ...
    if (info.device) reinterpret_cast<IUnknown *>(info.device)->Release();
    if (info.d3d8)   reinterpret_cast<IUnknown *>(info.d3d8)->Release();
}
...
void get_d3d9_offsets(d3d9_offsets *offsets)
{
    ...
    if (info.swap) info.swap->Release();
    if (info.device) info.device->Release();
    if (info.d3d9ex) info.d3d9ex->Release();
}
...
void get_d3d12_offsets(d3d12_offsets *offsets)
{
    ...
    if (info.swap) info.swap->Release();
    if (info.queue) info.queue->Release();
}
